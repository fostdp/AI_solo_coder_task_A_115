...
fn compute_derivatives(
    ...
    let drag_mag = if rel_vel_mag > 0.001 {
        0.5 * input.air_density_kgm3 * rel_vel_mag * rel_vel_mag * cd * cross_section / input.projectile_mass_kg
    } else {
        0.0
    };
...
pub fn simulate_ballistics(input: &BallisticInput) -> BallisticResult {
    ...
    let diameter = input.projectile_diameter_m;

    let mut state = State {
        x: 0.0,
        y: input.launch_height_m,
        z: 0.0,
        vx: input.initial_velocity * angle_rad.cos(),
        vy: input.initial_velocity * angle_rad.sin(),
        vz: 0.0,
    };
    ...
    let (_, initial_mach, initial_cd) = compute_flow_state(&state, input, wind_x, wind_z, diameter);
    ...
    while state.y >= -1.0 {
        let (new_state, error, mach, cd) = rk4_step(
            &state, input, wind_x, wind_z, cross_section, diameter, dt
        );
    ...
    let impact_kinetic_energy = 0.5 * input.projectile_mass_kg * vel_mag * vel_mag;
